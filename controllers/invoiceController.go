...
func GetInvoices() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while fetching all invoice"})
		}

		var allowInvoices []bson.M
		if err = result.All(ctx, &allowInvoices); err != nil {
			log.Fatal(err)
		}
		c.JSON(http.StatusOK, allowInvoices)
	}
}
...
func GetInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// err := invoiceCollection.FindOne(ctx, bson.M{"nvoice_id": invoiceId}).Decode(&invoice)
		// defer cancel()
		// if err != nil {
...
func CreateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		validationErr := validate.Struct(invoice) // validate all post requessts
